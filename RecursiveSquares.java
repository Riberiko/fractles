 ...
 * 
 * Description: This program generates a fractle
 *
...
public class RecursiveSquares extends JPanel {
    ...
    private void drawULFigure (Graphics g, int cx, int cy, int size, int depth) {
        ...
        if (depth <= 0) return;
        drawULFigure(g, cx - size/2, cy - size/2, size/2, depth-1);

        drawURFigure(g, cx + size/2, cy - size/2, size/2, depth-1);
        drawLLFigure(g, cx - size/2, cy + size/2, size/2, depth-1);
        drawLRFigure(g, cx + size/2, cy + size/2, size/2, depth-1);
