 ...
 * 
 * Description: This program generate a unique version of a fractle
 *
...
public class RecursiveSquares2 extends JPanel {
    ...
    public static final Color MY_CUSTOM_COLOR = new Color(105, 1, 8);
    ...
    private void drawULFigure (Graphics g, int cx, int cy, int size, int depth) {
        ...
        if (depth > 0){
            drawULFigure(g, cx, cy - size/2, size/2, depth-1);
    
            drawURFigure(g, cx + size/2, cy, size/2, depth-1);
            drawLLFigure(g, cx, cy + size/2, size/2, depth-1);
            drawLRFigure(g, cx - size/2, cy, size/2, depth-1);
        }

        if(depth%2 == 0){
            g.setColor(MY_CUSTOM_COLOR);
        }else{
    ...
    // Draw the figure in the upper-right corner
    // the figure is centered at (cx, cy), size is the size of the inner square,
    // and depth is the current recursion depth
    private void drawURFigure (Graphics g, int cx, int cy, int size, int depth) {

        if (depth > 0){
            drawURFigure(g, cx, cy - size/2, size/2, depth-1);
    
            drawULFigure(g, cx - size/2, cy, size/2, depth-1);
            drawLLFigure(g, cx, cy + size/2, size/2, depth-1);
            drawLRFigure(g, cx + size/2, cy, size/2, depth-1);
        }
    ...
    // Draw the figure in the lower left corner 
    // the figure is centered at (cx, cy), size is the size of the inner square,
    // and depth is the current recursion depth
    private void drawLLFigure (Graphics g, int cx, int cy, int size, int depth) {

        if (depth > 0){
            drawLLFigure(g, cx - size/2, cy, size/2, depth-1);
    
            drawULFigure(g, cx, cy - size/2, size/2, depth-1);
            drawURFigure(g, cx + size/2, cy, size/2, depth-1);
            drawLRFigure(g, cx, cy + size/2, size/2, depth-1);
        }
	...
	if (args.length > 0) {
        ...
        frame.setContentPane(new RecursiveSquares2());
        frame.pack();
